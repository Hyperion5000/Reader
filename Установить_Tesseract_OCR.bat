...
echo.
echo Скачиваю русские и английские языковые файлы OCR в локальную папку tessdata...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$root = '%~dp0'; $dir = Join-Path $root 'tessdata'; New-Item -ItemType Directory -Force -Path $dir | Out-Null; foreach ($lang in @('eng','rus','osd')) { $target = Join-Path $dir ($lang + '.traineddata'); if (!(Test-Path $target)) { Invoke-WebRequest -Uri ('https://github.com/tesseract-ocr/tessdata_fast/raw/main/' + $lang + '.traineddata') -OutFile $target } }"
